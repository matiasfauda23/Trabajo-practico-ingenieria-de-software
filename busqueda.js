...
function iniciarMapa() {
    ...
    miMapa = L.map('mapa').setView([-34.52, -58.70], 13);
...
function listarTalleres() {
    ...
    const nombreBuscado = normalizarTexto(document.getElementById("filtro-nombre").value);
    ...
    const talleresFiltrados = talleres.filter(taller => {
        ...
        const coincideNombre = !nombreBuscado || nombreTaller.includes(nombreBuscado) || nombreColaborador.includes(nombreBuscado);
        const coincideCategoria = !categoriaSeleccionada || taller.rubro === categoriaSeleccionada;
